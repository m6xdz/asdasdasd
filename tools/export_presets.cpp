...
int main(){
 ...
 const char* names[]={"Legit","Rage","Relax Legit","Relax Rage","Tap","Replay"};
 ...
 for(int i=0;i<6;++i){config::settings_t s;s.lab_enabled=true;
  ...
  std::ostringstream cfg;config::serialize_settings(cfg,names[i],s);
  out.push_back({{"id","beta-standard-"+std::to_string(i)},{"name",names[i]},{"description",descriptions[i]},{"channel",i==4?"lab":"stable"},{"cfg",cfg.str()}});
 }
 std::cout<<out.dump(2)<<'\n';
}
