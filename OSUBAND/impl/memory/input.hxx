...
namespace input {
    ...
    inline std::atomic<HWND> g_target_hwnd{nullptr};
    inline std::atomic<unsigned> desktop_generation{1};
    ...
    inline virtual_desktop_t& virtual_desktop( ) {
        thread_local virtual_desktop_t vd{};
        thread_local unsigned generation=0;
        const auto current=desktop_generation.load(std::memory_order_relaxed);
        if(generation!=current){vd.valid=false;generation=current;}
        return vd;
    }
    ...
    inline void invalidate_virtual_desktop( ) {
        desktop_generation.fetch_add(1,std::memory_order_relaxed);
    }
