...
namespace ui {
    ...
    public:
        ...
        std::optional<POINT> handle_mouse(POINT p,const MSLLHOOKSTRUCT& raw) {
        ...
        void set_account(const std::string& name,const std::string& plan,bool lab,const std::string& avatar="",const std::string& id="") {m_studio.user=name;m_studio.plan=plan;m_studio.user_id=id;m_avatar_url=avatar;m_lab_access=lab;m_authorized=true;}
        void set_authorized(bool value,bool lab,uint64_t deadline=UINT64_MAX) {m_auth_deadline=deadline;m_authorized=value;m_lab_access=lab;}
    ...
    private:
        mutable std::recursive_mutex m_settings_mutex;
