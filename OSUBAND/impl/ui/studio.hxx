...
struct model {
    ...
    bool cloud_busy=false,submit=false,can_undo=false;
};
struct actions {
    bool changed=false, save=false, load=false, refresh=false, folder=false;
    bool browse_replay=false, load_replay=false, close=false, pause=false, website=false;
    bool undo=false;
};
inline void text(ImDrawList* d,ImVec2 p,const char* s,float size=16,ImU32 color=0) {
    d->AddText(size>20&&heading_font?heading_font:ImGui::GetFont(),size,p,color?color:ink,tr(s));
}
...
inline bool button(const char* id,const char* label,ImVec2 p,ImVec2 size,bool primary=false,bool enabled=true) {
    ...
    d->AddRectFilled(p,ImVec2(p.x+size.x,p.y+size.y),fill,9);
    if(!primary)d->AddRect(p,ImVec2(p.x+size.x,p.y+size.y),border,9);
...
inline bool toggle(const char* id,const char* label,bool& value,ImVec2 p,float w,const char* note=nullptr) {
    ImGui::SetCursorScreenPos(p);bool changed=ImGui::InvisibleButton(id,ImVec2(w,note?51:31));
    if(changed)value=!value;
    auto* d=ImGui::GetWindowDrawList();text(d,p,label,16);
    if(note)text(d,ImVec2(p.x,p.y+25),note,13,muted);
    ImVec2 a(p.x+w-42,p.y),b(a.x+40,a.y+22);
    d->AddRectFilled(a,b,value?rose:IM_COL32(67,65,78,255),11);
    d->AddCircleFilled(ImVec2(a.x+(value?29:11),a.y+11),7,value?base:ink,24);
    return changed;
}
inline bool slider(const char* label,const char* id,float& value,float lo,float hi,ImVec2 p,float w,const char* format="%.2f") {
    ...
    ImGui::SetCursorScreenPos(ImVec2(p.x,p.y+25)); ImGui::SetNextItemWidth(w);
...
inline void input(const char* label,char* data,size_t n,ImVec2 p,float w) {
    ImGui::SetCursorScreenPos(p);ImGui::SetNextItemWidth(w);ImGui::InputText(label,data,n);
}
...
inline void apply_theme() {
    ...
    base=prefs.theme==2?IM_COL32(237,240,248,255):IM_COL32(14,15,23,255);
    panel=prefs.theme==2?IM_COL32(255,255,255,255):IM_COL32(24,26,38,255);
    ink=prefs.theme==2?IM_COL32(27,30,44,255):IM_COL32(242,244,251,255);
    muted=prefs.theme==2?IM_COL32(91,99,120,255):IM_COL32(158,167,190,255);
    border=prefs.theme==2?IM_COL32(35,45,65,30):IM_COL32(185,204,255,27);
    ...
    if(prefs.theme==2){
        s.Colors[ImGuiCol_FrameBg]=ImVec4(.87f,.89f,.94f,1);
        s.Colors[ImGuiCol_FrameBgHovered]=ImVec4(.81f,.84f,.91f,1);
        s.Colors[ImGuiCol_FrameBgActive]=ImVec4(.77f,.81f,.90f,1);
        s.Colors[ImGuiCol_PopupBg]=ImVec4(.97f,.98f,1,1);
        s.Colors[ImGuiCol_Header]=ImVec4(.94f,.81f,.87f,1);
        s.Colors[ImGuiCol_HeaderHovered]=ImVec4(.95f,.86f,.91f,1);
        s.Colors[ImGuiCol_Button]=ImVec4(.87f,.89f,.94f,1);
    }
...
inline void preference_controls(ImVec2 p,float w){
    ...
    if(button("##ru","Русский",ImVec2(p.x,p.y+30),ImVec2(w/2-6,35),prefs.russian)){prefs.russian=true;dirty=true;}
    if(button("##en","English",ImVec2(p.x+w/2+6,p.y+30),ImVec2(w/2-6,35),!prefs.russian)){prefs.russian=false;dirty=true;}
    text(d,ImVec2(p.x,p.y+84),"Theme",16);
    const char* themes[]={"Rose","Ocean","Light"};for(int i=0;i<3;++i)if(button(themes[i],themes[i],ImVec2(p.x+i*w/3,p.y+110),ImVec2(w/3-8,34),prefs.theme==i)){prefs.theme=i;dirty=true;}
    dirty|=toggle("##motion","Animations",prefs.animations,ImVec2(p.x,p.y+165),w);
    dirty|=slider("Animation speed","##motion-speed",prefs.motion,.25f,2.f,ImVec2(p.x,p.y+205),w,"%.2fx");
...
inline actions draw(model& m,config::settings_t& s) {
    ...
    ImVec2 origin(std::max(0.f,(io.DisplaySize.x-width)/2)+m.offset.x,std::max(0.f,(io.DisplaySize.y-height)/2)+m.offset.y);
    ImGui::SetNextWindowPos(origin,ImGuiCond_Always);ImGui::SetNextWindowSize(ImVec2(width,height),ImGuiCond_Always);
    ...
    auto* d=ImGui::GetWindowDrawList();auto P=[&](float x,float y){return ImVec2(origin.x+x,origin.y+y);};
    card(d,origin,ImVec2(width,height),base,18);
    d->AddRectFilledMultiColor(P(1,1),P(width-1,94),panel,panel,base,base);
    ImGui::SetCursorScreenPos(P(0,0));ImGui::InvisibleButton("##drag",ImVec2(880,83));
    if(ImGui::IsItemActive()){m.offset.x+=io.MouseDelta.x;m.offset.y+=io.MouseDelta.y;}
    brand(d,P(28,24),44);text(d,P(85,29),"OSU BAND",26);text(d,P(86,58),"BETA",11,rose);
    ...
    if(!m.lab_allowed&&m.page==2)m.page=0;
    const char* pages[]={"Play","Profiles","LAB","System"};int page_slot=0;
    for(int i=0;i<4;++i){if(i==2&&!m.lab_allowed)continue;if(button(pages[i],pages[i],P(28+page_slot++*124,99),ImVec2(112,39),m.page==i))m.page=i;}
    ...
    if(m.page==0) {
        ...
        for(int i=0;i<5;++i){float y=266+i*67.f;
            ImGui::SetCursorScreenPos(P(28,y));if(ImGui::InvisibleButton(names[i],ImVec2(247,57)))m.module=i;
        ...
        if(m.module==0){
            ...
            if(s.aim_legit_mode){ImGui::SetCursorScreenPos(P(700,552));ImGui::SetNextItemWidth(367);a.changed|=ImGui::SliderFloat("##clampval",&s.aim_legit_clamp,1.05f,1.8f,"Clamp %.2f",ImGuiSliderFlags_AlwaysClamp);}
        }else if(m.module==1){
        ...
        text(d,P(28,180),"Cloud configs",28);
        if(button("##refresh","Refresh",P(810,180),ImVec2(130,37),false,!m.cloud_busy))a.refresh=true;
        if(button("##web","Open website",P(952,180),ImVec2(140,37)))a.website=true;
        const char* collections[]={"From modeof19","From players","Mine"};
        for(int i=0;i<3;++i)if(button(collections[i],collections[i],P(28+i*212,235),ImVec2(202,36),m.collection==i)){m.collection=i;m.selected=-1;}
        card(d,P(28,285),ImVec2(636,314));
        ImGui::SetCursorScreenPos(P(40,295));ImGui::BeginChild("##cloudlist",ImVec2(612,294));
        bool any=false;
        for(size_t i=0;i<m.profiles.size();++i){const auto& profile=m.profiles[i];
            if(m.collection==0&&(!profile.official||profile.status!="published"))continue;
            if(m.collection==1&&(profile.official||profile.status!="published"))continue;
            if(m.collection==2&&profile.owner_id!=m.user_id)continue;
            any=true;const auto p=ImGui::GetCursorScreenPos();auto* row=ImGui::GetWindowDrawList();
            ImGui::PushID(static_cast<int>(i));if(ImGui::InvisibleButton("##config",ImVec2(599,115)))m.selected=static_cast<int>(i);
            card(row,p,ImVec2(599,107),m.selected==static_cast<int>(i)?ImGui::GetColorU32(ImGuiCol_Header):panel,10);
            if(i<m.profile_avatars.size()&&m.profile_avatars[i])row->AddImageRounded(m.profile_avatars[i],ImVec2(p.x+12,p.y+12),ImVec2(p.x+46,p.y+46),ImVec2(0,0),ImVec2(1,1),IM_COL32_WHITE,17);
            else brand(row,ImVec2(p.x+12,p.y+12),34);
            row->PushClipRect(ImVec2(p.x+56,p.y),ImVec2(p.x+425,p.y+58),true);
            text(row,ImVec2(p.x+56,p.y+10),profile.name.c_str(),17);
            const auto by=profile.author+" · v"+std::to_string(profile.revision);text(row,ImVec2(p.x+56,p.y+35),by.c_str(),12,muted);row->PopClipRect();
            text(row,ImVec2(p.x+438,p.y+12),profile.style.c_str(),13,rose);
            const char* status=profile.status=="pending"?"On review":profile.status=="rejected"?"Rejected":profile.status=="published"?"Published":"Private";
            text(row,ImVec2(p.x+438,p.y+35),status,12,muted);
            row->PushClipRect(ImVec2(p.x+12,p.y+56),ImVec2(p.x+584,p.y+101),true);
            text(row,ImVec2(p.x+12,p.y+60),profile.description.c_str(),13,muted);
            if(!profile.review_note.empty())text(row,ImVec2(p.x+12,p.y+82),profile.review_note.c_str(),12,rose);
        ...
        text(d,P(708,354),"Style",13,muted);ImGui::SetCursorScreenPos(P(708,375));ImGui::SetNextItemWidth(360);
        ImGui::BeginDisabled();ImGui::Combo("##style",&m.style,"Legit\0Rage\0Relax Legit\0Relax Rage\0Relax + Aim\0Aim + Assist\0Autobot\0Tap\0Replay\0");ImGui::EndDisabled();
        ...
        if(button("##undo","Undo",P(236,613),ImVec2(196,36),false,m.can_undo))a.undo=true;
        text(d,P(446,623),"Changes apply between maps",13,muted);
    }else if(m.page==2){
        text(d,P(28,181),"Beta",28);
        card(d,P(28,250),ImVec2(518,355));card(d,P(566,250),ImVec2(526,355));
        text(d,P(52,276),"Session",22);
        a.changed|=toggle("##hud","Session HUD",s.hud_enabled,P(52,330),470);
        s.lab_enabled=true;
        text(d,P(52,398),m.paused?"MODULES PAUSED":"Modules ready",17,m.paused?rose:ink);
        text(d,P(52,445),"F8  /  Pause all modules",16,muted);
        if(button("##undo-beta","Undo last config",P(52,532),ImVec2(470,42),false,m.can_undo))a.undo=true;
        text(d,P(590,276),"Cloud sync",22);
        text(d,P(590,330),m.cloud_busy?"Loading...":"Checks every 15 seconds",16,muted);
        text(d,P(590,384),"Changes apply between maps",16);
        text(d,P(590,431),"Previous settings remain available with Undo",14,muted);
        if(button("##beta-site","Open website",P(590,532),ImVec2(478,42)))a.website=true;
    }else{
        ...
        a.changed|=toggle("##capture","Exclude menu from capture",s.stream_proof,P(52,550),470);
        ...
        text(d,P(590,440),"F4  /  Show or hide menu",17);text(d,P(590,473),"F8  /  Pause all modules",17);
        text(d,P(590,519),m.user.substr(0,36).c_str(),17);text(d,P(590,548),m.plan.substr(0,52).c_str(),14,muted);
        if(button("##account","Open account",P(914,549),ImVec2(152,36)))a.website=true;
    }
