...
namespace threads {
    ...
    public:
        c_cache( ) = default;
        ~c_cache( ) { stop( ); }
        [[nodiscard]] bool healthy( ) const { return m_running.load( ); }
        ...
        void start( ) {
            ...
            m_process_thread = std::thread( [ this ] { guarded("process", [this]{process_loop();}); } );
            m_game_thread = std::thread( [ this ] {
                // TIME_CRITICAL made the reader compete with osu!lazer and the
                // desktop compositor.  Above-normal is enough for timely
                // snapshots without starving the game or other applications.
                ::SetThreadPriority( ::GetCurrentThread( ), THREAD_PRIORITY_ABOVE_NORMAL );
                guarded("game", [this]{game_loop();});
            } );
            m_beatmap_thread = std::thread( [ this ] { guarded("beatmap", [this]{beatmap_loop();}); } );
        }
        ...
        osu::full_snapshot_t get_ui_snapshot() const {
            std::shared_lock lock(m_mutex);
            osu::full_snapshot_t out;out.game=m_snapshot.game;
            out.beatmap.loaded=m_snapshot.beatmap.loaded;out.object_count=m_snapshot.beatmap.objects.size();
            return out;
        }
        ...
        void invalidate_beatmap_cache( ) {
            std::unique_lock source_lock( m_source_mutex );
    ...
    private:
        // Lifecycle lock precedes m_mutex. Never close a process or destroy
        // its client while either reader is using it.
        mutable std::shared_mutex m_source_mutex;
        template<class Fn> void guarded(const char* name, Fn fn) {
            try { fn(); }
            catch (const std::exception& e) { dbg::log("worker %s stopped: %s",name,e.what()); m_running=false; }
            catch (...) { dbg::log("worker %s stopped: unknown exception",name); m_running=false; }
        }
        ...
        void process_loop( ) {
            ...
            while ( m_running ) {
                ...
                const bool attached = m_process.valid( ) && m_client != nullptr;
                source_lock.unlock();
                std::this_thread::sleep_for( attached ? 250ms : 1000ms );
            }
        }
        ...
        void game_loop( ) {
            ...
            while ( m_running ) {
                bool in_play = false;
                std::shared_lock source_lock( m_source_mutex );
                if ( m_client && m_process.valid( ) ) {
                    ...
                    source_lock.unlock();
                    module_tick_fn tick;
                    { std::lock_guard lock(m_module_tick_mutex); tick=m_module_tick; }
                    // Release held input on detach, not only on the next map.
                    if (tick) tick(osu::game_snapshot_t{}, osu::beatmap_data_t{});
                }

                // A zero-duration sleep is effectively a busy loop on Windows.
        ...
        void beatmap_loop( ) {
            ...
            while ( m_running ) {
                ...
                source_lock.unlock();
                // A missing/unreadable map must not trigger 500 full parses/s.
                std::this_thread::sleep_for( in_play && !beatmap_loaded ? 100ms : 50ms );
            }
        }
    };
