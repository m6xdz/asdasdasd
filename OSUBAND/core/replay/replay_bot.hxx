...
namespace replay {
    ...
    public:
        ...
        void update( const osu::game_snapshot_t& game, const osu::beatmap_data_t&, bool user_control = false ) {
            if(!enabled || !m_replay.valid || game.cur_state!=osu::game_state_t::play){release_keys();m_driving_cursor=false;return;}
            ...
            if(frames.size()<2 || game_time<frames.front().absolute_time){release_keys();return;}
            ...
            if(m_frame_index>=static_cast<int>(frames.size())-1){release_keys();m_driving_cursor=false;return;}
            ...
            if(move_cursor)input::move_absolute_virtual_desktop(target_p.x,target_p.y);

            if(!parse_buttons)release_keys();
            if ( parse_buttons ) {
