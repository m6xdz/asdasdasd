...
void collect(){if(!job.valid()||job.wait_for(std::chrono::milliseconds(0))!=std::future_status::ready)return;auto r=job.get();view.busy=false;
 ...
 if(r.kind==3){session=r.data.at("session");view.connected=true;view.authorized=session.value("authorized",false);view.user=session.value("account",cloud::json::object()).value("displayName","OSU!BAND member");view.can_stable=session.value("canStable",view.authorized);view.can_beta=session.value("canBeta",false);view.lab_access=view.can_beta;
  if(view.lab_channel&&!view.can_beta){view.lab_channel=false;view.message="Beta is unavailable on a Stable subscription.";}
...
void handle(const band_loader::actions& a){
 ...
 if(a.logout){client.disconnect();view.connected=false;view.authorized=false;view.can_stable=false;view.can_beta=false;session={};release={};avatar_url.clear();view.avatar=0;view.message="This loader has disconnected.";}
 ...
 if(a.launch){if(view.lab_channel&&!view.can_beta){view.message="Beta requires a Beta subscription. Stable access cannot launch Beta.";return;}if(!view.lab_channel&&!view.can_stable){view.message="Stable requires an active subscription.";return;}if(!view.osu_running){view.message="Start osu!lazer before launching OSU!BAND.";return;}if(process_running(L"OSUBAND.exe")){view.message="OSU!BAND is already running. Press F4 to open its menu.";return;}if(!release.is_object()||!release.contains("sha256")){view.message="No release is published for this channel.";return;}
